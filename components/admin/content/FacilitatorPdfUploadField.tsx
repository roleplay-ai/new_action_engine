...
import { uploadFacilitatorPdf } from "@/lib/facilitator-pdf-upload";

export function FacilitatorPdfUploadField({
  cohortId,
  onUploaded,
  disabled,
  label = "Upload PDF",
  ...
  cohortId: string;
  onUploaded: (pdfUrl: string, pdfName: string) => void;
  disabled?: boolean;
  label?: string;
}) {
  ...
  async function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    setUploading(true);
    try {
      const { pdfUrl, pdfName } = await uploadFacilitatorPdf(cohortId, file);
      setFileName(pdfName);
