  ...
  if (loading) {
    return (
      <div className="cohort-admin-detail-loading" aria-busy="true">
        <Loader2 size={20} className="cohort-admin-spin" /><span>Loading members…</span>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="cohort-admin-alert cohort-admin-alert--error cohort-admin-alert--inner" role="alert">
        <AlertCircle size={17} />
        <div><strong>Could not load members</strong><span>{error}</span></div>
      </div>
    );
  }

  return <MembersClient cohortId={cohortId} initialRoster={roster} tags={tags} />;
}
      ...
      <header className="cohort-admin-header">
        <div>
          <div className="cohort-admin-company-heading">
            ...
            <div><h1>Members & tags</h1><strong>Batch roster</strong></div>
          </div>
          <p>Create tags, assign them to participants, and manage the roster the same way a trainer does.</p>
        </div>
      </header>
        ...
        <main className="cohort-admin-workspace">
          {selectedCohort ? (
            ...
            <div className="cohort-admin-placeholder">
              <span><Tag size={25} /></span>
              <h2>Select a batch</h2>
              <p>Choose a batch to create tags and assign them to its members.</p>
            </div>
          )}
        </main>
      </div>
