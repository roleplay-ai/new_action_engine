...
## 1. Tables (final schema, after migrations 001–020)
...
| completed_at | TIMESTAMPTZ | null = still due |
| created_at | TIMESTAMPTZ | |

Indexed on `(user_action_id)` and `(user_id, scheduled_at)`. One row per individual habit-loop rep.
...
- **`action_status`**: `scheduled`, `success`, `failed`, `skipped`, `habit_started`, `cemented` — note: the TypeScript type layer (`lib/types.ts`) additionally lists a client-side `'pending'` value that **does not exist** as a real database state; don't expect to see it in a `status` column.
- **`event_type`**: `ACCEPTED`, `SUCCESS`, `HABIT_STARTED`, `CEMENTED`, `DECLINED` — the type layer also lists `READ`/`SCHEDULED`, but no code path currently inserts either.
...
| actions | same company, or superadmin | company admin (own company) or superadmin | same | same |
...
| habit_occurrences | self | self | self | self |
...
- **`/api/cron/email-scheduler`** — the only cron doing real work. Auth via `Authorization: Bearer <CRON_SECRET>` or a `?secret=` query param (skipped if `CRON_SECRET` isn't set). Finds due `email_schedules` rows, sends via the shared SendGrid helper (logging each send to `email_campaign_logs`), advances `next_run_at`, and deactivates one-time (`specific_date`) schedules after they fire.
